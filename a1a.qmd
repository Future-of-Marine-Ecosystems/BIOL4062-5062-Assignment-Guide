...
2\. The species data scales vary greatly across variables

3\. The species all have relatively different distributions, none of which are normal.

Are these issues? How do we fix them?

## Transformations

Look back at the PCA lecture. What are potential problems with PCA? 1. Covariance Matrix PCA requires data to be in the same units 2. Normality is desirable, but not essential 3. Precision is desireable, but not essential 4. Many zeroes in the data
...
log(0) returnes negative infinity. That's going to be a problem later in our analysis. We can fix that by adding a small increment. Keep in mind though that each species has a very different data scale in this dataset, and adding an inappropriate increment could cause us trouble later:

```{r}
# Test variance effects of different increments
var(log(data_test$limpet)[-1])
var(log(data_test$limpet + 1))
var(log(data_test$limpet + 0.001))
var(log(data_test$limpet + 0.000000001))

# Test boxplots of different increments
boxplot(log(data_test$limpet),
...
The PCA loadings are the correlation between the variables and each component. Here, we see bluefin and squid are negatively correlated with component 1, while mackerel, sardine, and limpet are weakly correlated with component 1. We can continue this type of interpretation through the other components as well.
...
Scores are the value of each data point on each principle component. Lets try plotting them:
